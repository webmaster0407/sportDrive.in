                  ...
                  <div class="col-sm-3">
                    <div class="form-group">
                        ...
                        @foreach($allOrderStatus as $oStatus)
                            <option value="{{$oStatus->status_id}}" @if($byOrderStatus == $oStatus->status_id) selected @endif >{{$oStatus->status}}</option>
                        @endforeach
                        @endif
                        </select>
                    </div>
                  ...
                  <div class="col-sm-3">
                    <div class="form-group">
                        ...
                        @foreach($allPaymentStatus as $pStatus)
                            <option value="{{$pStatus->status_id}}" @if($byPaymentStatus == $pStatus->status_id) selected @endif >{{$pStatus->status}}</option>
                        @endforeach
                        @endif
                        </select>
                    </div>
                      ...
                      <?php $i = 1;  ?>
                        @foreach ($orders as $Orders)
                          <tr >
                            ...
                            <td>@if($customer!= null){{$customer->first_name}} {{$customer->last_name}}@else{{$Orders->first_name}} {{$Orders->last_name}}@endif</td>
                             <td @if($orderStatus[$Orders->id]['status']=="Failed") style="color: red" @endif >@if($orderStatus[$Orders->id]['status'] == "Pending") {{"Order Received"}}@else {{$orderStatus[$Orders->id]['status']}} @endif</td>
                             <td @if($paymentStatus[$Orders->id]['slug']=="failed") style="color: red" @endif >{{$paymentStatus[$Orders->id]['status']}}</td>
                             <td>{{number_format($Orders->total,2)}}</td>
                            <td><?php echo  date('d-M-Y H:i:s',strtotime($Orders->order_date)) ?>
                             </td>
                          </tr>
                        <?php $i++; ?>
                        @endforeach
