    ...
    public function paymentRequest(Request $request, $order_id) {
        try {
            ...
            if($order->is_payment_proceed == "Y"){//if previous order is proceeded to gateway and we did not received any response
                $carts = Cart::where("order_id",$order_id)->where("is_deleted","N")->get()->toArray();
            ...
            foreach ($cartData as $cart) {
                ...
                $color = null;
                $size = null;
                if (count($cart->colorId) > 0) {
                    $color = $cart->color[0];
                }
                if (count($cart->sizeId) > 0) {
                    $size = $cart->size[0];
                }
            ...
            return view('user.payu-request-form')->with(compact("hash", "posted", "action", "shippingAddress"));
        } catch (Exception $ex) {
            $data = [
                'input_params' => $request->all(),
