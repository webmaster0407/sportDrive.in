...
class ProductController extends Controller
{
    ...
    public function ListCategoryProduct(Requests\ListCategoryProductRequest  $request,$category){
        try{
            ...
            $allProductIdsData = $subCategories = $subSubCategories =$subCategoriesIds = $productIds = $subSubCategoriesIds = $subCategoriesProductIds =$subSubCategoriesProductIds = array();
            ...
            if($category->level_id==0){//if the category is main parent then fetch all the products of sub subcategories
                $subCategoriesIds = Category::where('parent_id',$category->id)->pluck("id");
                $subCategoriesProductIds = ProductCategoryMap::whereIn("category_id",$subCategoriesIds)->pluck("product_id")->toArray();
                $subSubCategoriesIds = Category::whereIn('parent_id',$subCategoriesIds)->pluck("id");
            ...
            $allProductIdsData = array_unique(array_merge($allProductIdsData,$productIds,$subCategoriesProductIds,$subSubCategoriesProductIds));

            //get category products Filters
            if(count($subCategoriesIds)>0)
                $subCategories = Category::whereIn('id',$subCategoriesIds)->where("is_active","Y")->select("id","name")->get();
            if(count($subSubCategoriesIds)>0)
                $subSubCategories = Category::whereIn('id',$subSubCategoriesIds)->where("is_active","Y")->select("id","name")->get();
            $selected = null;
            if($data!=null){//user refreshed page with selected filters to hits directly filter range
                ...
                if(array_key_exists("f",$data)) {//check for like men, women, girl or boys
                    ...
                    $allProductIdsData = ProductFor::whereIn("for", $forArray)->whereIn("product_id", $allProductIdsData)->pluck("product_id")->toArray();
                }
                $categoryArray = null;
                if(array_key_exists("sc",$data) && array_key_exists("ssc",$data) ){
                ...
                $query = Product::where("is_active","Y")->where('is_completed','Y')->where('is_verified','Y');
                ...
                $allProducts = Product::whereIn("id",$products) ->where("is_active","Y")->where('is_completed','Y')->orderby($orderBy,$sortOrderType)->whereIn("id",$allProductIdsData)->get();
                $products = Product::whereIn("id",$products) ->where("is_active","Y")->where('is_completed','Y')->orderby($orderBy,$sortOrderType)->whereIn("id",$allProductIdsData)->paginate($take);
                $productsMax = $allProducts->max("price");
                $productsMin = $allProducts->min("price");
                $productsAvg = $allProducts->avg("price");
            }else{
                $allProducts = Product::whereIn("id",$allProductIdsData)->where("is_active","Y")->where('is_completed','Y')->orderby($orderBy,$sortOrderType)->get();
                $productsMax = $allProducts->max("price");
                $productsMin = $allProducts->min("price");
                $productsAvg = $allProducts->avg("price");
    ...
    public function productDetails(Request $request, $slug){
        try{
                ...
                /*check the product is_active,is_completed or not #start modified on 20/08/2018*/
                if($user!= null){
                    $ispresent = RatingReviews::where('product_id', $product->id)->where('email',$user->email_address)->first();
                    if( $ispresent!= null){
                        $flag = true;
                    }else{
                        $flag = false;
                ...
                $getattributesSize = ProductConfiguration::select('product_configuration.AttributeSize','attributes.name','product_configuration.quantity')->where('product_id',$product->id)->join('attributes','attributes.id','=','product_configuration.AttributeSize')->get();
                ...
                $getattributesColor = ProductConfiguration::select('product_configuration.product_id','product_configuration.AttributeColor','product_configuration.image as colorImage','attributes.*')->where('product_id',$product->id)->join('attributes','attributes.id','=','product_configuration.AttributeColor')->get();
                ...
                $offer = ProductsOffers::join("offers","products_offers.offer_id","=","offers.id")->where("offers.is_active","Y")->where("product_id",$product->id)->first();
                if($offer!=null)
                    $productOffers = ProductsOffers::join("products","products_offers.product_id","=","products.id")->where("offer_id",$offer->id)->whereNotIn("product_id",array($product->id))->groupBy("products.id")->select("products.id","products.name","products.slug","products.image","products.sku","products.discount_price","products.price")->get();
                $brandName = Brand::where('id',$product->brand_id)->first();
                return view("user.product-details")->with(compact('product','slug','productConfiguration','getattributesColor','getattributesSize' ,'ratingReviews','pConfiguration','ratingAvg','totalRatings','flag','user','finalprice','brandName',"offer","productOffers"));
        }catch(\Exception $e){
            $data = [
                'input_params' => $slug,
    ...
    public function displaySizeConfig(Request $request){
        try{
                ...
                $getattributesSize = ProductConfiguration::select('AttributeSize','quantity')->where('product_id',$data['pid'])->where('AttributeColor',$data['color'])->get();
    ...
    public function displayPriceConfig(Request $request){
        try{
                ...
                $getprice = ProductConfiguration::select('price')->where('product_id',$data['pid'])->where('AttributeColor',$selectedColor)->where('AttributeSize', $selectedSize)->first();
