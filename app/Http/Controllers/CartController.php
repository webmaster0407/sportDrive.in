...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request,$product_id){
        try{
            ...
            if(array_key_exists("selectedColor",$data) && $data['selectedColor']!=null && array_key_exists("selectedSize",$data) && $data['selectedSize']!=null){// if color and size both selected
            ...
            }elseif(array_key_exists("selectedColor",$data) && $data['selectedColor']!=null){// if only color is selected
                $configData = ProductConfiguration::where('product_id',$product_id)
                    ->where('AttributeColor',$data['selectedColor'])
                ...
                $configurationId = $configData->id;
                $productQty = $configData->quantity;
            }elseif(array_key_exists("selectedSize",$data) && $data['selectedSize']!=null){// if only size is selected
                $configData = ProductConfiguration::where('product_id',$product_id)
            ...
            if($user!=null){
                ...
                if($totalIncompleteOrder>0){
                    ...
                    if($totalIncompleteOrder>1){
                        ...
                        Order::whereIn("id",$orders->toArray())->update(array("is_completed"=>"Y","is_payment_proceed"=>"Y","order_status"=>11,"payment_status"=>11,"is_abandoned"=>"Y"));
                    }
            ...
            if($order!= null){//if configured product and there is already unplaced order
                $cart = Cart::where('order_id',$order->id)->where('product_id',$product_id)->where('configuration_id',$configurationId)->where('is_deleted','N')->first();
            }
            ...
            if(array_key_exists("otherQuantity",$data)){
                ...
                foreach($data['otherQuantity'] as $key=>$otherProductQty){
                    if($otherProductQty['quantity']>0){
                        $configData = ProductConfiguration::where('product_id',$key)->where('AttributeSize',$data['selectedSize'])->first();
                        if($configData!=null && $configData->quantity>=$otherProductQty['quantity']){
                            $configurationId = $configData->id;
                            $cart = Cart::where('order_id',$order->id)->where('product_id',$key)->where('configuration_id',$configurationId)->where('is_deleted','N')->first();
                            if(count($cart)<=0){
                                $cartData = [
                                    'order_id' => $order->id,
                                    'product_id' => $key,
    ...
    public function viewCart(){
        try{
            ...
            $errorMessageArray = array();
            $shipping_charge = 100;
            return view("user.cart-view")->with(compact('cartData','shipping_charge',"errorMessageArray","offersPrices"));
        }catch(\Exception $e){
            $data = [
